...
export const getAllContacts = async (req, res) => { 
    try {
        ...
        console.error(
          "(getAllContacts in Controller): Error in getting all Contact:",
          error
        );
...
export const getMessagesByUserId = async (req, res) => { 
    try {
        ...
        console.error("(getMessages in Controller): Error fetching Messages:", error);
        res.status(500).json({ message: "Internal Server Error" });
    }
}
...
export const sendMessage = async (req, res) => {
    try {
        ...
        console.error("sendMessage() in Controller: Error sending message", error);
        res.status(500).json({ message: "Internal server error" });
    }
}
...
export const getChatPartners = async (req, res) => { 
    try {
        ...
        console.error("(getChatPartners in Controller): Error fetching chat partners:", error);
        res.status(500).json({ message: "Internal Server Error" });
    }
}